...
def spectra_comparison(upper_ev=5):
    ...
    for ax, n in zip(axs.T, [3, 4]):
        ...
        for j, (g2, eta, _) in enumerate(POINTS[n]):
            ev = eigvals(g2, eta, 0.4, n, n, m=50, n_eigvals=n + upper_ev)
            ax[0].scatter(np.real(ev[:n]), np.imag(ev[:n]), c=f'C{j}', marker=MARKERS[j], label=rf'$\tau_n = {lbl[j]}$')
            ax[0].scatter(np.real(ev[n:]), np.imag(ev[n:]), edgecolors=f'C{j}', marker=MARKERS[j], facecolors='none')
    ...
    plt.tight_layout(pad=0.02, rect=[0, 0, 1, 0.9])
